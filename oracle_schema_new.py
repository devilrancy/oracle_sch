...
try:
	...
	print serr
	print """\nModule: sqlparse not found. Please make sure the module is installed before using this program.
		This is a lexar parser which is used for parsing the statements.
		"""
	exit()
try:
	import cx_Oracle
			...
			if ',' in column_string:
				for item in column_string.split(','):
					if item[-1] != '.':
						...
						return_list.extend([str(table_name + item.strip() + '.').strip()])
					else:
						# print table_name + item.strip()
						return_list.extend([str(table_name + item.strip()).strip()])
			else:
				# print table_name + column_string
				return_list.extend([str(table_name + column_string).strip()])
		return return_list
...
def extract_index_comp_info(parsed_sql):
		...
		return_list.extend([str('INDEX.' + str(index_name) + '.ON.' + str(parsed_sql.get_name()) + '.' + \
								re.findall(r'(?<=\().*?(?=,\))', str(definition))[0].strip() + '.').strip()])
		return return_list

def main_generator(file_r):
		SQL = file_r.read()
...
def main():
	...
	if '--only-compare' in sys.argv:
		...
		try:
			res_f = open(res_fn, 'w')
			res_f.write("==========> COMPARISION RESULTS <==========\n")
